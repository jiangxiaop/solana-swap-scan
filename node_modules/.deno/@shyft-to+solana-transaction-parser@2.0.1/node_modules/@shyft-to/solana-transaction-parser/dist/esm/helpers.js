...
export function flattenParsedTransaction(transaction) {
    const result = [];
    if (!transaction) {
        return result;
    }
...
function generateLogsParsingRegex() {
    ...
    const prepareLineForRegex = (l) => l.replaceAll("{}", ".*").replaceAll("(", "\\(").replaceAll(")", "\\)").replaceAll("]", "\\]").replaceAll("[", "\\[");
    const regexTemplate = `\
...
function programEnter(context, invokedProgram, invokeLevel) {
...
function programExit(context, exitedProgram) {
    ...
    if (lastProgram != exitedProgram)
        throw new Error("[ProramExit] callstack mismatch");
    context.immediate.currentDepth -= 1;
...
export function parseLogs(logs) {
    ...
    const getCurrentCallId = (c) => c.callIds[c.callIds.length - 1];
    const getCurrentProgram = (c) => c.callStack[c.callStack.length - 1];
