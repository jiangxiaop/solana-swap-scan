...
export class DexParser {
  ...
  public formatAllTrades(parseResult: ParseResult): ResSwapStruct[] {
    const trades = parseResult.trades.map(trade =>
  ...
  public async parsePerBlock(blockNumber: number, config?: ParseConfig): Promise<ParseResult[]> {
    ...
    if (transactions) {
      ...
      const parseResult = validTransactions.map((transaction) =>
  ...
  public parseBlockData(blockData: VersionedBlockResponse, blockNumber: number): ParseResult[] {
    let start = Date.now();
