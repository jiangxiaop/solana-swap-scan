 ...
 */
export async function getTokenInfoUseCache(tokenAddress: string): Promise<TokenInfo | null> {
    try {
        const tokenIncaseAddress = tokenAddress.toLowerCase();
        const cacheData = await redisClient.hget(tokenInfoCache, tokenIncaseAddress);
        if (cacheData) {
        ...
        if ((result as any).affectedRows > 0) {
            return await getTokenInfoFromDB(tokenIncaseAddress);
        }
 ...
 */
export async function getTokenInfoFromDB(tokenAddress: string): Promise<TokenInfo | null> {
    const sql = `
  ...
  `;
    try {
        ...
        if (dbData) {
            await redisClient.hset(tokenInfoCache, tokenAddress, JSON.stringify(dbData));
            return dbData;
        }
        return null;
    } catch (error) {
