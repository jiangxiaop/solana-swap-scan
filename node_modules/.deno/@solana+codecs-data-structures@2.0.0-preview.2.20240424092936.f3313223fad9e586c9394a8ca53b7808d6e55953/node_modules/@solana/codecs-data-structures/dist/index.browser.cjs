...
function getNullableEncoder(item, config = {}) {
  ...
  const encoder = getUnionEncoder(
    [
      ...
      codecsCore.transformEncoder(getTupleEncoder([prefix, item]), (value) => [1, value])
    ],
...
function getNullableDecoder(item, config = {}) {
  ...
  const decoder = getUnionDecoder(
    [
      codecsCore.transformDecoder(prefix, (_value) => null),
...
function getZeroableNullableEncoder(item, config = {}) {
  const zeroValue = getZeroValue(item.fixedSize, config.zeroValue);
  return getUnionEncoder(
    [codecsCore.transformEncoder(codecsCore.fixEncoderSize(getBytesEncoder(), item.fixedSize), (_value) => zeroValue), item],
    (variant) => Number(variant !== null)
  );
}
function getZeroableNullableDecoder(item, config = {}) {
  const zeroValue = getZeroValue(item.fixedSize, config.zeroValue);
  return getUnionDecoder(
    [codecsCore.transformDecoder(codecsCore.fixDecoderSize(getBytesDecoder(), item.fixedSize), () => null), item],
    (bytes, offset) => codecsCore.containsBytes(bytes, zeroValue, offset) ? 0 : 1
  );
}
function getZeroableNullableCodec(item, config = {}) {
  return codecsCore.combineCodec(getZeroableNullableEncoder(item, config), getZeroableNullableDecoder(item, config));
}
function getZeroValue(itemSize, zeroValue) {
  const zeroValueOrDefault = zeroValue ?? new Uint8Array(itemSize).fill(0);
  if (zeroValueOrDefault.length !== itemSize) {
    throw new errors.SolanaError(errors.SOLANA_ERROR__CODECS__EXPECTED_ZERO_VALUE_TO_MATCH_ITEM_FIXED_SIZE, {
...
exports.getZeroableNullableCodec = getZeroableNullableCodec;
exports.getZeroableNullableDecoder = getZeroableNullableDecoder;
exports.getZeroableNullableEncoder = getZeroableNullableEncoder;
