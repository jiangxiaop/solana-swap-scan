...
// 读取token元数据文件
async function readTokenMetadata(): Promise<{ [key: string]: TokenInfo }> {
  try {
    await ensureDataDirectory();
    const data = await Deno.readTextFile(TOKEN_METADATA_FILE);
...
// 写入token元数据文件
async function writeTokenMetadata(metadata: { [key: string]: TokenInfo }) {
  await ensureDataDirectory();
  await Deno.writeTextFile(TOKEN_METADATA_FILE, JSON.stringify(metadata, null, 2));
}

// 读取token价格文件
async function readTokenPrices(): Promise<TokenPrice[]> {
  try {
    await ensureDataDirectory();
    const data = await Deno.readTextFile(TOKEN_PRICES_FILE);
    return JSON.parse(data);
  } catch {
    return [];
  }
...
// 写入token价格文件
async function writeTokenPrices(prices: TokenPrice[]) {
  await ensureDataDirectory();
  await Deno.writeTextFile(TOKEN_PRICES_FILE, JSON.stringify(prices, null, 2));
}
...
async function getTokenInfo(mint: string) {
  ...
  try {
    ...
    if (metadata[mint]) {
      tokenInfoCache[mint] = metadata[mint];
    ...
    if (token) {
      ...
      const currentMetadata = await readTokenMetadata();
      currentMetadata[mint] = tokenInfo;
      await writeTokenMetadata(currentMetadata);
...
async function estimateUsdValue(mint: string, amount: number, decimals: number, timestamp: number) {
  try {
    ...
    const recentPrices = prices
      .filter(p => p.mint === mint && Math.abs(p.timestamp - timestamp) < 3600)
    ...
    if (currentPrice) {
      ...
      const allPrices = await readTokenPrices();
      allPrices.push(newPrice);

      // 只保留最近7天的价格数据
      const sevenDaysAgo = Math.floor(Date.now() / 1000) - (7 * 24 * 60 * 60);
      const filteredPrices = allPrices.filter(p => p.timestamp > sevenDaysAgo);
      await writeTokenPrices(filteredPrices);
